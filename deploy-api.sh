...
# 用法: ./deploy-api.sh -e <env> [-s <service>] [-d <source_dir>] [-b <branch>]
...
${BOLD}Prod 流程:${RESET}
  請先執行 ./tag-api.sh -t <version> 完成合併與打 tag，
  再執行 ./deploy-api.sh -e prod，會自動取用最新 tag 作為 image tag。
...
if [[ "$ENV" == "prod" ]]; then
    ...
    [[ -z "$TAG_VERSION" ]] && error "找不到任何 git tag，請先執行 ./tag-api.sh -t <version>"
